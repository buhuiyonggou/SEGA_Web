...
def test(word2vec_model, graph_data):
    print("Extracting embeddings for nodes...")
    ...
    print(f"Initial train_indices length: {np.sum(train_mask.cpu().numpy())}")
    print(f"Initial test_indices length: {np.sum(test_mask.cpu().numpy())}")
    ...
    print(f"Filtered train_indices length: {len(train_indices)}")
    print(f"Filtered test_indices length: {len(test_indices)}")

    if len(train_indices) == 0 or len(test_indices) == 0:
        print("Training or test set is empty.")
        return 0  # No accuracy to calculate
